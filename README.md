...
- 강의의 `/go-live` 단계에서 에이전트가 **내 계정**에 GitHub 레포·Vercel·Supabase·Sentry를 만들어 연결합니다. 그 뒤로 코드와 서비스는 전부 내 소유의 클라우드에 남고, Codespace는 지워도 됩니다.
- `/go-live` 전까지는 작업이 이 Codespace 안에만 있습니다 — 그 전엔 지우지 마세요. GitHub은 30일간 쓰지 않은 Codespace를 자동 삭제하니, 오래 쉬게 된다면 `/go-live`까지는 진행해 두는 것이 안전합니다.
- 나중에 설치 가능한 컴퓨터가 생기면: 강의의 한 줄 설치 명령으로 워크스페이스를 만든 뒤 `web/`을 내 GitHub 레포의 클론으로 바꾸면 됩니다 — 에이전트에게 부탁하면 해 줍니다.

## 알아둘 것

- **다음에 다시 올 때는 새로 만들지 말고 이어서 여세요** — Code → Codespaces 목록에 이미 만든 것이 보입니다. 그걸 클릭하면 작업하던 그대로 열립니다. 새로 만들면 빈 워크스페이스가 하나 더 생겨 "내 작업이 사라진" 것처럼 보입니다.
